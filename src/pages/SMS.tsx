    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">SMS Notification Center</h1>
            <p className="text-slate-500">Communicate with your patients instantly</p>
          </div>
        </div>
      ...
      {!isConnected && (
        <Card className="bg-orange-50 border-orange-200 border-2 border-dashed">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">SMS Service Disabled</h3>
              <p className="text-slate-600 max-w-md mx-auto">To enable SMS notifications, you must first configure your Dialog eSMS gateway credentials.</p>
            </div>
            <Button onClick={() => setShowConfig(true)} className="gap-2 shadow-lg shadow-emerald-200">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 transition-opacity duration-300 ${!isConnected ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="md:col-span-1 space-y-6">
          <Card className={recipientType === 'individual' ? 'border-emerald-500 ring-2 ring-emerald-500/10' : ''}>
            <button 
              className="w-full p-6 text-left"
              onClick={() => setRecipientType('individual')}
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-lg ${recipientType === 'individual' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                  <Users className="h-5 w-5" />
                </div>
                <span className="font-bold text-slate-900">Individual</span>
              </div>
              <p className="text-sm text-slate-500">Send message to a specific patient</p>
            </button>
          </Card>

          <Card className={recipientType === 'bulk' ? 'border-emerald-500 ring-2 ring-emerald-500/10' : ''}>
            <button 
              className="w-full p-6 text-left"
              onClick={() => setRecipientType('bulk')}
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-lg ${recipientType === 'bulk' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                  <MessageSquare className="h-5 w-5" />
                </div>
                <span className="font-bold text-slate-900">Bulk Broadcast</span>
              </div>
              <p className="text-sm text-slate-500">Send message to ALL registered patients</p>
            </button>
          </Card>
            ...
            <CardContent className="space-y-2">
              {[
                ...
                <button 
                  ...
                  className="w-full px-3 py-2 text-left text-xs font-medium hover:bg-white/10 rounded transition-colors border border-white/10"
                >
                  {tmp}
                </button>
              ))}
        ...
        <div className="md:col-span-2">
              ...
              <form onSubmit={handleSend} className="space-y-6">
                {recipientType === 'individual' && (
                  <div className="space-y-4 pb-6 border-b border-slate-100">
                    <label className="text-sm font-medium text-slate-700">Find Patient</label>
                    ...
                    {selectedPatient && (
                      <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => setSelectedPatient(null)} className="text-slate-400 hover:text-red-500">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    )}
                ...
                {recipientType === 'bulk' && (
                  <div className="p-4 bg-orange-50 border border-orange-100 rounded-lg flex items-center gap-3 mb-6">
                    <AlertCircle className="h-5 w-5 text-orange-600" />
                    <p className="text-sm text-orange-700 font-medium">Warning: This will send an SMS to all patients in the system.</p>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-end">
                    ...
                    <span className={`text-[10px] font-bold uppercase ${message.length > 160 ? 'text-red-500' : 'text-slate-400'}`}>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-xl border border-slate-300 p-4 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition-all"
                ...
                <div className="flex gap-3">
                  <Button 
                    type="submit" 
                    className="flex-1 h-12 text-lg gap-2 shadow-lg shadow-emerald-200"
                    isLoading={isSending}
          ...
          <Card className="mt-8">
            <CardHeader 
              ...
              action={<Button variant="ghost" size="sm" className="gap-2"><History className="h-4 w-4" /> Logs</Button>}
            />
            <CardContent>
              <div className="space-y-4">
                {logs.length > 0 ? (
                  logs.map((log, i) => (
                    <div key={log._id || i} className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100">
