...
export const Queue = () => {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'waiting': return 'bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800';
      case 'with doctor': return 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800';
      case 'completed': return 'bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Today's Token Queue</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Live monitoring and management of patient flow</p>
        </div>
        <Button 
          variant="outline" 
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
              ...
              {searchResult && (
                <div className="mt-5 p-4 bg-white dark:bg-slate-800 rounded-xl border border-emerald-200 dark:border-emerald-800/50 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="font-bold text-sm text-slate-900 dark:text-white">{searchResult.name}</p>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{searchResult.phone}</p>
                    </div>
                  </div>
                  <Button 
          ...
          <Card>
            <CardContent className="p-5">
              <h4 className="font-bold text-sm text-slate-900 dark:text-white mb-4">Queue Summary</h4>
              <div className="space-y-2">
                <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  <span className="text-xs text-slate-600 dark:text-slate-400">Total Waiting</span>
                  <span className="font-bold text-sm text-slate-900 dark:text-white">{queue.filter((q: any) => q.status === 'waiting').length}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  <span className="text-xs text-slate-600 dark:text-slate-400">In Consultation</span>
                  <span className="font-bold text-sm text-slate-900 dark:text-white">{queue.filter((q: any) => q.status === 'with doctor').length}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  <span className="text-xs text-slate-600 dark:text-slate-400">Completed Today</span>
                  <span className="font-bold text-sm text-slate-900 dark:text-white">{queue.filter((q: any) => q.status === 'completed').length}</span>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                      <th className="px-6 py-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Token</th>
                      <th className="px-6 py-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Patient Name</th>
                    ...
                    {isLoading ? (
                      ...
                      queue.map((entry: any) => (
                        <tr key={entry._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                          ...
                          <td className="px-6 py-3">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase ${getStatusStyle(entry.status)}`}>
                              {entry.status}
                            </span>
                          </td>
                      ...
                      <tr>
                        <td colSpan={4} className="px-6 py-16 text-center text-slate-400 dark:text-slate-500">
                          ...
                          <p className="text-sm italic">The queue is currently empty</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
